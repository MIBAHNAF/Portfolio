  ...
  return (
    <div id='about' className='w-full px-4 sm:px-6 md:px-8 lg:px-10 xl:px-[10%] py-8 sm:py-10 scroll-mt-20 relative z-40'>
      ...
      <div className="my-8 sm:my-10 lg:my-12 max-w-7xl mx-auto">
        <div className="relative z-30 grid grid-cols-1 lg:grid-cols-[minmax(240px,300px)_minmax(0,1fr)] xl:grid-cols-[minmax(280px,340px)_minmax(0,1fr)] items-stretch gap-8 lg:gap-10 xl:gap-14">
          <div ref={imageRef} className={`w-full max-w-64 sm:max-w-72 lg:max-w-none aspect-[3/4] mx-auto lg:mx-0 rounded-3xl relative overflow-hidden ${slideInLeft} ${imageVisible ? slideInLeftVisible : slideInLeftHidden}`}>
          ...
          <div ref={descRef} className={`min-w-0 w-full h-full flex flex-col ${slideInRight} ${descVisible ? slideInRightVisible : slideInRightHidden}`}>
            ...
            <div ref={infoRef} className={`flex-1 ${fadeInUp} ${infoVisible ? fadeInUpVisible : fadeInUpHidden}`}>
              <ul className='grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 w-full h-full items-stretch relative'>
              {infoList.map(({ icon, iconDark, title, description }, index) => {
                if (title === 'Certifications') {
                  return (
                    <li
                      key={index}
                      className={`relative flex flex-col justify-center border-[0.5px] ${isDark ? 'border-gray-600 bg-gray-800' : 'border-gray-400 bg-white'} rounded-xl p-4 sm:p-5 cursor-pointer transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:shadow-lg h-full ${isPopoverOpen('Certifications') ? 'z-30' : 'z-10'}`}
                      onMouseEnter={() => handleMouseEnter('Certifications')}
                      onMouseLeave={handleMouseLeave}
                      onClick={() => handlePopoverToggle('Certifications')}
                    >
                      <div>
                        <MdWorkspacePremium className={`w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 mt-1 ${isDark ? 'text-gray-200' : 'text-gray-700'}`} />
                        <h3 className={`my-3 lg:my-4 font-semibold text-base lg:text-lg xl:text-xl ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{title}</h3>
                        <p className={`text-sm lg:text-base leading-5 lg:leading-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{description}</p>
                      </div>
                      {isPopoverOpen('Certifications') && (
                        <>
                          <div className="absolute left-0 right-0 h-4 top-full z-[99998]" />
                          <div style={popoverPanelStyle} className={`absolute right-0 top-full z-[99999] mt-4 ${isDark ? 'bg-gray-800/95 border-gray-600' : 'bg-white/95 border-gray-200'} backdrop-blur-md rounded-xl p-5 sm:p-6 shadow-2xl border animate-fade-in flex flex-col justify-center items-center text-center`}>
                            <Image src={assets.comptia} alt='CompTIA Security+' className='w-28 mb-4' />
                            <h3 className={`text-2xl font-bold font-Ovo mb-3 ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{title}</h3>
                            <div className={`text-base sm:text-lg font-semibold mb-2 font-Ovo ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>CompTIA Security+ (SY0-701)</div>
                            <div className={`mb-1 text-base ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Issued: <span className={`font-bold font-Ovo ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>Sep 2025</span></div>
                            <div className={`mb-5 text-base ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Expires: <span className={`font-bold font-Ovo ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>Sep 2028</span></div>
                            <a
                              href='https://www.credly.com/badges/a57e33ad-27a0-425a-9fea-56de7c270c8c'
                              target='_blank'
                              rel='noopener noreferrer'
                              className={`px-6 py-2 border rounded-full ${isDark ? 'border-gray-500 bg-gray-700/80 text-gray-200 hover:bg-gray-600/80' : 'border-gray-500 bg-white/80 text-gray-700 hover:bg-gray-50/80'} inline-flex items-center gap-2 backdrop-blur-sm transition-all duration-300 w-fit mx-auto mt-2 shadow-sm hover:scale-105 hover:shadow-md text-base`}
                            >
                              Verify on Credly
                ...
                if (title === 'Education') {
                  return (
                    <li
                      key={index}
                      className={`relative flex flex-col justify-center border-[0.5px] ${isDark ? 'border-gray-600 bg-gray-800' : 'border-gray-400 bg-white'} rounded-xl p-4 sm:p-5 cursor-pointer transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:shadow-lg h-full ${isPopoverOpen('Education') ? 'z-30' : 'z-10'}`}
                      onMouseEnter={() => handleMouseEnter('Education')}
                      onMouseLeave={handleMouseLeave}
                      onClick={() => handlePopoverToggle('Education')}
                    >
                      <div>
                        <Image src={isDark ? iconDark : icon} alt={title} className='w-7 sm:w-8 lg:w-10 mt-1' />
                        <h3 className={`my-3 lg:my-4 font-semibold text-base lg:text-lg xl:text-xl ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{title}</h3>
                        <p className={`text-sm lg:text-base leading-5 lg:leading-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{description}</p>
                      </div>
                      {isPopoverOpen('Education') && (
                        <>
                          <div className="absolute left-0 right-0 h-4 top-full z-[99998]" />
                          <div style={popoverPanelStyle} className={`absolute left-1/2 top-full z-[99999] -translate-x-1/2 mt-4 ${isDark ? 'bg-gray-800/95 border-gray-600' : 'bg-white/95 border-gray-200'} backdrop-blur-md rounded-xl p-5 sm:p-6 shadow-2xl border animate-fade-in flex flex-col justify-center items-center text-center`}>
                            <Image src={assets.umass_logo} alt='UMass Boston Logo' className='w-20 mb-4' />
                            <h3 className={`text-2xl font-bold font-Ovo mb-3 ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{title}</h3>
                            <div className={`text-base sm:text-lg font-semibold mb-2 font-Ovo ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>University of Massachusetts Boston</div>
                            <div className={`mb-1 text-base ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Graduated: <span className={`font-bold font-Ovo ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>May 2026</span></div>
                            <div className={`mb-2 text-base ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Computer Science, Bachelor of Science</div>
                            <div className={`mb-4 text-base ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>GPA: <span className={`font-bold font-Ovo ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>3.932</span></div>
                            <button
                ...
                if (title === 'Projects') {
                  return (
                    <li
                      key={index}
                      className={`relative flex flex-col justify-center border-[0.5px] ${isDark ? 'border-gray-600 bg-gray-800' : 'border-gray-400 bg-white'} rounded-xl p-4 sm:p-5 cursor-pointer transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:shadow-lg h-full ${isPopoverOpen('Projects') ? 'z-30' : 'z-10'}`}
                    ...
                    >
                      ...
                      {isPopoverOpen('Projects') && (
                        <>
                          <div className="absolute left-0 right-0 h-4 top-full z-[99998]" />
                          <div style={popoverPanelStyle} className={`absolute left-1/2 top-full z-[99999] -translate-x-1/2 mt-4 ${isDark ? 'bg-gray-800/95 border-gray-600' : 'bg-white/95 border-gray-200'} backdrop-blur-md rounded-xl p-5 sm:p-6 shadow-2xl border animate-fade-in flex flex-col`}>
                              ...
                              {projectsData.map((proj) => (
                                ...
                                >
                                  <div className='flex flex-col items-start'>
                                    <span className={`text-base sm:text-lg font-semibold font-Ovo ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{proj.name}</span>
                                    <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{proj.type} &bull; {proj.hours} hrs</span>
                                  </div>
