  ...
  return (
    <div className='w-11/12 max-w-3xl text-center mx-auto h-screen flex flex-col items-center justify-center gap-4 pt-20 sm:pt-24 lg:pt-0'>
      <div ref={imageRef} className={`${fadeInUp} ${imageVisible ? fadeInUpVisible : fadeInUpHidden}`}>
        ...
        <h1 ref={titleRef} className={`text-3xl sm:text-6xl font-Ovo lg:text-[66px] ${isDark ? 'text-white' : 'text-black'} ${fadeInUp} ${titleVisible ? fadeInUpVisible : fadeInUpHidden}`} style={{ animationDelay: '0.4s' }}>Software Engineer and IT Specialist</h1>
        ...
        <div ref={buttonsRef} className={`flex flex-col sm:flex-row gap-4 mt-4 ${fadeInUp} ${buttonsVisible ? fadeInUpVisible : fadeInUpHidden}`} style={{ animationDelay: '0.8s' }}>
            <a href = "#contact" 
            className='px-10 py-3 border border-gray-500 rounded-full bg-white text-black flex items-center gap-2 transition-all duration-300 hover:scale-105 hover:shadow-md'
            > contact me <Image src = {assets.right_arrow} alt='' className='w-4'/></a>
            <a href = "/SWE_Resume.pdf" download 
            className={`px-10 py-3 border rounded-full border-gray-500 flex items-center gap-2 transition-all duration-300 hover:scale-105 hover:shadow-md ${isDark ? 'text-white' : 'text-black'}`}
            > SWE Resume <Image src = {isDark ? assets.right_arrow_white : assets.download_icon} alt='' className='w-4'/></a>
            <a href = "/IT_Specialist Resume.pdf" download 
            className={`px-10 py-3 border rounded-full border-gray-500 flex items-center gap-2 transition-all duration-300 hover:scale-105 hover:shadow-md ${isDark ? 'text-white' : 'text-black'}`}
            > IT Resume <Image src = {isDark ? assets.right_arrow_white : assets.download_icon} alt='' className='w-4'/></a>
        </div>
